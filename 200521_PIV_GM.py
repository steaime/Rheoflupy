...
if __name__ == '__main__':
    ...
    if ('-skipplot' in cmd_list or plot_root is None):
        ...
        plt_track = SharedFunctions.ConfigGet(config, 'plot', 'plot_track', False, bool)
        if (plt_track):
            from scipy.interpolate import UnivariateSpline
            def load_track(fname, col_x=5, col_y=6, col_t=7, px_size=1.0, dt=1.0):
                res_x = []
                res_y = []
                res_t = []
                with open(fname) as fin:
                    for line in fin:
                        try:
                            words = line.split()
                            res_x.append(float(words[col_x])*px_size)
                            res_y.append(float(words[col_y])*px_size)
                            res_t.append(float(words[col_t])*dt)
                        except:
                            pass
                return res_x, res_y, res_t
            x1, y1, t1 = load_track('E:\\191127_Crack9\\JunctionVideo\\Track_Tip1.txt')
            x2, y2, t2 = load_track('E:\\191127_Crack9\\JunctionVideo\\Track_Tip2.txt')
            x3, y3, t3 = load_track('E:\\191127_Crack9\\JunctionVideo\\Track_Tip3.txt')
            y1 = np.subtract(imShape[1], y1)
            y2 = np.subtract(imShape[1], y2)
            y3 = np.subtract(imShape[1], y3)
            spl_x1 = UnivariateSpline(t1, x1, k=4, s=1400)
            spl_y1 = UnivariateSpline(t1, y1, k=4, s=4000)
            spl_x2 = UnivariateSpline(t2, x2, k=4, s=10000)
            spl_y2 = UnivariateSpline(t2, y2, k=4, s=3000)
            spl_x3 = UnivariateSpline(t3, x3, k=4, s=500)
            spl_y3 = UnivariateSpline(t3, y3, k=4, s=2000)
            #spl_vx1_p = UnivariateSpline(t1, x1, k=4, s=1400/2).derivative()
            #spl_vy1_p = UnivariateSpline(t1, y1, k=4, s=4000/2).derivative()
            #spl_vx2_p = UnivariateSpline(t2, x2, k=4, s=10000/2).derivative()
            #spl_vy2_p = UnivariateSpline(t2, y2, k=4, s=3000/2).derivative()
            #spl_vx3_p = UnivariateSpline(t3, x3, k=4, s=100000/2).derivative()
            #spl_vy3_p = UnivariateSpline(t3, y3, k=4, s=100000/2).derivative()
            x1s, y1s = spl_x1(t1), spl_y1(t1)
            x2s, y2s = spl_x2(t2), spl_y2(t2)
            x3s, y3s = spl_x3(t3), spl_y3(t3)
            spl_vx1, spl_vy1 = spl_x1.derivative(), spl_y1.derivative()
            spl_vx2, spl_vy2 = spl_x2.derivative(), spl_y2.derivative()
            spl_vx3, spl_vy3 = spl_x3.derivative(), spl_y3.derivative()
            vx1, vy1 = spl_vx1(t1), spl_vy1(t1)
            vx2, vy2 = spl_vx2(t2), spl_vy2(t2)
            vx3, vy3 = spl_vx3(t3), spl_vy3(t3)
            v1 = np.sqrt(vx1**2+vy1**2)
            v2 = np.sqrt(vx2**2+vy2**2)
            v3 = np.sqrt(vx3**2+vy3**2)
            spl_ax1, spl_ay1 = spl_vx1.derivative(), spl_vy1.derivative()
            spl_ax2, spl_ay2 = spl_vx2.derivative(), spl_vy2.derivative()
            spl_ax3, spl_ay3 = spl_vx3.derivative(), spl_vy3.derivative()
            ax1, ay1 = spl_ax1(t1), spl_ay1(t1)
            ax2, ay2 = spl_ax2(t2), spl_ay2(t2)
            ax3, ay3 = spl_ax3(t3), spl_ay3(t3)
        ...
        for tidx in range(min(len(imSeq), len(refinedv_data_binned[0]))):
            ...
            if (plt_track):
                vel_scale = 40.0
                acc_scale = 4000.0
                tip_kw = {'markersize':20, 'markeredgecolor':'k', 'markeredgewidth':1}
                vel_kw = {'color':'g', 'lw':6, 'head_width':18, 'head_length':27}
                acc_kw = {'color':'y', 'lw':4, 'head_width':12, 'head_length':18}
                if (tidx*bin_z < t3[0]):
                    cur_idx = int(tidx*bin_z)
                    if (cur_idx < len(x1s)):
                        ax.plot([x1s[cur_idx]], [y1s[cur_idx]], 'go', **tip_kw)
                        ax.arrow(x1s[cur_idx], y1s[cur_idx], vel_scale*vx1[cur_idx], vel_scale*vy1[cur_idx], **vel_kw)
                        ax.arrow(x1s[cur_idx], y1s[cur_idx], acc_scale*ax1[cur_idx], acc_scale*ay1[cur_idx], **acc_kw)
                    if (cur_idx < len(x2s)):
                        ax.plot([x2s[cur_idx]], [y2s[cur_idx]], 'go', **tip_kw)
                        ax.arrow(x2s[cur_idx], y2s[cur_idx], vel_scale*vx2[cur_idx], vel_scale*vy2[cur_idx], **vel_kw)
                        ax.arrow(x2s[cur_idx], y2s[cur_idx], acc_scale*ax2[cur_idx], acc_scale*ay2[cur_idx], **acc_kw)
                else:
                    cur_idx = int(tidx*bin_z-t3[0])
                    if (cur_idx < len(x3s)):
                        ax.plot([x3s[cur_idx]], [y3s[cur_idx]], 'go', **tip_kw)
                        ax.arrow(x3s[cur_idx], y3s[cur_idx], vel_scale*vx3[cur_idx], vel_scale*vy3[cur_idx], **vel_kw)
                        ax.arrow(x3s[cur_idx], y3s[cur_idx], acc_scale*ax3[cur_idx], acc_scale*ay3[cur_idx], **acc_kw)
